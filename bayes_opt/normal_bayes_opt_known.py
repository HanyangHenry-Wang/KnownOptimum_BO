...
from bayes_opt.gp_known import GaussianProcessKnown
...
class NormalBayesOpt_Known:

    def __init__(self, func, SearchSpace,fstar=0, acq_name="ei",verbose=1):
        """      
        Input parameters
        ----------
        ...
        self.gp=GaussianProcessKnown(self.scaleSearchSpace,verbose=verbose)
    ...
    def select_next_point(self):
        ...
        """
        fstar_scaled=(self.fstar-np.mean(self.Y_ori))/np.std(self.Y_ori)
        self.gp=GaussianProcessKnown(SearchSpace=self.scaleSearchSpace,fstar=fstar_scaled,verbose=self.verbose)
